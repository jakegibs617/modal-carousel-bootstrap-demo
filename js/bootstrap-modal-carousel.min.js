...
!function(a){
	...
	{fit:function(b) {
		if(b||(b=a(".item.active", this.$element).height()),b!=this.$element.height()&&(a.support.transition?this.$element.animate({
			height:b}):this.$element.height(b),this.$element.trigger(a.Event("fit.bs.carousel", {height:b}))), 1==this.$element.find(".item.active img").length) {

				var c=this.$element.find(".item.active img"), d={
		...
		a(document).on("slide.bs.carousel", ".carousel.carousel-fit",
				
		function(b){
			var c=a(this).data("bs.carousel");
				c.$element.height(a(".item.active",
				c.$element).height());

			var d=a(".item.active",
				c.$element)["left"==b.direction?"next":"prev"]();

			d.length||(d=a(".item:"+("left"==b.direction?"first":"last")+"-child",
				c.$element));
			...
			e=d.height(),
			c.fit(e)
		}).on("slid.bs.carousel", ".carousel.carousel-fit",
...
function(a){
	...
	function b(b,c){
		...
		var d=b.parents(".modal:first"), e=!d.hasClass("force-fullscreen"), f=d.height();
		
		e&&(f-=a(".modal-header", d).height(), f-=a(".modal-footer", d).height()), a.support.transition&&b.hasClass("slide")?b.animate({ marginTop:(f-c)/2}):b.css({marginTop:(f-c)/2})
...
function(a){
	"use strict";
	"function"==typeof define&&define.amd?define(["jquery", "bootstrap"],
	a):a(window.jQuery)
} (function(a) {
	"use strict";
	...
	{ 
		...
		detach:function(){
			a(this.options.local).trigger(a.Event("detach.bs.local")),
			this.$placeholder||(this.$placeholder=a("<div></div>").addClass("hidden").html(this.$element.find(".modal-body").html()).insertBefore(a(this.options.local))),
			this.$local=a(this.options.local).detach(),
			this.$element.find(".modal-body").empty().append(this.$local),
			a(this.options.local).trigger(a.Event("detached.bs.local"))
		},

		replace:function(){
			a(this.options.local).trigger(a.Event("replace.bs.local")),
			this.$local.detach().insertAfter(this.$placeholder),
			this.$element.find(".modal-body").html(this.$placeholder.html()),
			a(this.options.local).trigger(a.Event("replaced.bs.local"))
		}
	}),
	...
	}).on("click.bs.modal.data-api",'[data-toggle="modal"][data-local]',

	function(){
		a(a(this).attr("data-target")).modal("local",
			a(this).data("local"))
	})
});
